...
make_matrix <- function(x = matrix()) {
  ...
  inv <- NULL  ## restores to null the value of the inv (mean of the old vector no longer needed
  ...
  get <- function() {
    ## returns the vector x stored in the main function, doesn't require any input.
    x
  }
 
  setmean <- function(inverted_matrix) {
    ## store the value of the input (inverted_matrix) in a variable inv into the main function
    inv <<- inverted_matrix                               
  }
  getmean <- function() {  
    ## return value of the variable inv into the main function
    inv                                                   
  }
...
invert_matrix <- function(x, ...) {
  ...
  m <- mean(data, ...)        # compute the mean of data
  x$setmean(m)                # cache the mean, return the mean on the next line
  m
} 